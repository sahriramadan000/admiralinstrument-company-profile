...
<div class="nxl-content">
    <div class="main-content">
        <form action="{{ route('cms.project.update', $project->id) }}" class="row needs-validation" method="post" enctype="multipart/form-data" novalidate>
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input class="filepond image-input @error('image') is-invalid @enderror"
                                    ...
                                    name="image"
                                    accept="image/*"
                                    data-image="{{ $project->image ? asset('assets/img/project/' . $project->image) : '' }}">
                                @error('image')
                        ...
                        <div class="col-12 mt-2">
                            <div class="card border-top-0 br-15">
                                <div class="card-body">
                                    ...
                                    <p class="text-muted">Perbarui data proyek secara lengkap dan akurat.</p>

                                    <div class="mb-3">
                                        <label class="form-label fw-semibold">Nama Proyek</label>
                                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                            placeholder="Masukkan nama proyek Anda" value="{{ old('name', $project->name) }}" required>
                                        @error('name')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="category" class="form-control @error('category') is-invalid @enderror"
                                            placeholder="Masukkan kategori Anda" value="{{ old('category', $project->category) }}" required>
                                        @error('category')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
